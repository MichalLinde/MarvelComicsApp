...
import com.mlinde.marvelcomicsapp.util.TestRepo
import io.mockk.*
...
//        ComicDataWrapper("asd", "asd", 1, "asd", comicDataContainer, "asd", "asd")
    ...
    lateinit var testViewModel: ComicsListViewModel

    @Before
    override fun setup() {
        super.setup()
        testViewModel = ComicsListViewModel(repo)
    }


    @Test
    fun addition_isCorrect() = runTest {
        val data: ComicDataWrapper = mockk(relaxed = true)
        coEvery { repo.getComics() } returns data
        testViewModel.getComics()
        delay(1)
        assertEquals(data, testViewModel.comicsLiveData.value)
    }
}